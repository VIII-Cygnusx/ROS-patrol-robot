#! /home/cygnusx/miniconda3/envs/AI/bin/python3
import onnx
...
CLASSES = ['HG','SD','XM','YM','HG','SD','XM','YM']  # coco80类别
...
class Yolov5ONNX(object):
    ...
    def inference(self, img_path):
        ...
        img = img.astype(dtype=np.float16)  # onnx模型的类型是type: float32[ , , , ]
        img /= 255.0
...
if __name__ == "__main__":
    ...
    onnx_path = '/home/cygnusx/LINUX/ROS/my_gazebo_sim_pro/src/ONNX/yolov5n.onnx'
    model = Yolov5ONNX(onnx_path)
    # output, or_img = model.inference('data/images/img.png')
    output, or_img = model.inference('/home/cygnusx/LINUX/ROS/my_gazebo_sim_pro/src/ONNX/run.png')
    outbox = filter_box(output, 0.5, 0.5)  # 最终剩下的Anchors：0 1 2 3 4 5 分别是 x1 y1 x2 y2 score class
    if len(outbox) == 0:
        print('没有发现物体')
        sys.exit(0)
    or_img,type = draw(or_img, outbox)
